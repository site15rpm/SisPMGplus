...
export class SirconvDashboardModule {
    constructor(config) {
        ...
        this.masterData = {}; // Base JSON centralizada: { [ID]: { ...dados, audit, lastUpdate } }
    ...
    syncConvenio(id, newData) {
        ...
        if (!this.masterData[idStr]) {
            this.masterData[idStr] = {
                ID: idStr,
                lastUpdate: agora,
                audit: null,
                pendencias: []
        ...
        const entry = this.masterData[idStr];
    ...
    refreshConveniosList() {
        this.conveniosData = Object.values(this.masterData);
    }
    ...
    showDashboard() {
                ...
                <div class="sispmg-dashboard-main">
                    <div class="sispmg-dashboard-header" style="display: flex; justify-content: space-between; align-items: center; padding: 15px 20px; border-bottom: 1px solid #dcd3c5; background: #fdfaf6; position: sticky; top: 0; z-index: 100;">
                        ...
                        <div class="sispmg-dashboard-actions" style="display: flex; align-items: center; gap: 10px; flex-shrink: 0;">
                            <div id="sispmg-dashboard-bg-status" style="display: none; align-items: center; gap: 8px; color: #b3a368; font-size: 12px; margin-right: 15px; font-weight: 600; cursor: pointer; background: #fff; padding: 5px 12px; border-radius: 20px; border: 1px solid #dcd3c5;" title="Clique para parar">
                                <i class="fas fa-circle-notch fa-spin"></i>
                                <span style="text-decoration: underline;">Atualização em andamento...</span>
                            </div>
                            <button id="sispmg-dashboard-force-reload" class="sispmg-dashboard-btn sispmg-dashboard-btn-primary" title="Recarregar base Master">
                        ...
                        <table class="sispmg-dashboard-table">
                            <thead>
                                <tr>
                                    ...
                                    <th data-col="status" style="text-align: center;"><div class="sispmg-th-content" style="justify-content: center;">Status <i class="fas fa-filter sispmg-filter-trigger"></i></div></th>
                                </tr>
                            </thead>
                            <tbody id="sispmg-dashboard-tbody">
                                <tr><td colspan="12" style="text-align: center; padding: 40px;">Carregando dados da base master...</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
        ...
        modalContainer.querySelector('#sispmg-dashboard-force-reload').onclick = async () => {
            if (confirm("Deseja realmente limpar a base master e recarregar todos os dados do zero?")) {
                this.masterData = {};
        ...
        modalContainer.querySelectorAll('.sispmg-filter-trigger').forEach(trigger => {
            trigger.onclick = (e) => {
                e.stopPropagation();
                this.toggleFilterDropdown(trigger, trigger.closest('th').dataset.col);
            };
        });
    ...
    updateBackgroundStatus(isActive, message = "Atualização em andamento...") {
        ...
        if (reloadBtn) reloadBtn.style.display = isActive ? 'none' : 'inline-block';
    }
    ...
    showFilterSidebar() {
        ...
        sidebar.innerHTML = `
            <div style="display: flex; flex-direction: column; height: 100%; padding: 0;">
                <h2 style="color: #574e2d; font-size: 18px; border-bottom: 2px solid #b3a368; padding: 20px; margin: 0;"><i class="fas fa-filter"></i> Busca Avançada</h2>
    ...
    async processBackgroundQueue() {
        ...
        this.updateBackgroundStatus(true, `Atualização: 0/${total}`);
            ...
            try {
                ...
                if (auditData) {
                    ...
                    if (row) {
                        ...
                        tempDiv.innerHTML = this.renderRowHtml(this.masterData[convId]);
                        row.replaceWith(tempDiv.firstChild);
                    }
                }
            } catch (e) { console.error(e); }
            processed++;
            this.updateBackgroundStatus(true, `Atualização: ${processed}/${total}`);
    ...
    async performDeepAudit(convId, ignoreCache = false) {
        const entry = this.masterData[String(convId)];
        ...
        try {
            ...
            return { cronogramas, planoItens, historico, timestamp: Date.now(), lastUpdate: new Date().toLocaleString() };
    ...
    renderRowHtml(conv) {
        ...
        const vEst = parseFloat(conv.VALOR_ESTIMADO) || 0, vLiq = parseFloat(conv.LIQUIDADO) || 0, prog = vEst > 0 ? ((vLiq / vEst) * 100).toFixed(1) : 0;
        const colorT = vLiq > 0 ? (prog > 100 ? '#dc3545' : '#155724') : '#666';
        ...
        return `<tr class="sispmg-clickable-row ${isAudited ? 'sispmg-row-audited' : ''}" style="${rowStyle}" onclick="window.SisPMG_SirconvDashboard.loadAuditData('${conv.ID}')">
            ...
            <td class="sispmg-hide-on-audit" style="font-size: 11px;">${this.formatDate(conv.DTINICIAL)}<br>${this.formatDate(conv.DTFINAL)}</td>
            ...
            <td class="sispmg-hide-on-audit" style="text-align: right; font-size: 11px; color: #666;">${mPrev.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
            <td class="sispmg-hide-on-audit" style="text-align: right; font-size: 11px; font-weight: bold; color: #155724;">${mReal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
            ...
            <td style="text-align: center; white-space: nowrap;">
                <div style="display: flex; align-items: center; justify-content: center; gap: 5px;">
                    ...
                    <div style="display: flex; gap: 2px; width: 50px;">

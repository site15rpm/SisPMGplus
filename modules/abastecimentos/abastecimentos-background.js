...
const addLog = async (message, system = 'GERAL', type = 'info') => {
    ...
    chrome.runtime.sendMessage({ action: 'logsUpdated', logs: trimmedLogs }).catch(() => {});
};
...
const sendFinalizationSignal = () => {
     chrome.runtime.sendMessage({ action: 'extractionFinished' }).catch(() => {});
...
function downloadAsJson(data, fileName) {
    ...
    return new Promise((resolve, reject) => {
        chrome.downloads.download({ url: dataUrl, filename: fileName, saveAs: false }, id => chrome.runtime.lastError ? reject(chrome.runtime.lastError) : resolve(id));
    });
}
...
async function setupOffscreenDocument() { if (await chrome.offscreen.hasDocument?.()) return; await chrome.offscreen.createDocument({ url: OFFSCREEN_DOCUMENT_PATH, reasons: ['DOM_PARSER'], justification: 'Parse de tokens HTML.' }); }
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            if (config['file-native-excel']) {
                ...
                const downloadFolder = config['download-folder']?.trim() || 'Abastecimentos';
                const dateSuffix = `${formatDate.toYYYYMMDD(extractionData.params.startDate)}_a_${formatDate.toYYYYMMDD(extractionData.params.endDate)}`;
                await chrome.downloads.download({ url: dataUrl, filename: `${downloadFolder}/abastecimentos_prime_${dateSuffix}.xlsx`, saveAs: false });
            }
...
async function iniciarExtracaoSGTA(config, dateRange) {
    ...
    try {
        ...
        if (config['file-native-excel']) {
            ...
            const downloadFolder = config['download-folder']?.trim() || 'Abastecimentos';
            const dateSuffix = `${formatDate.toYYYYMMDD(extractionData.params.startDate)}_a_${formatDate.toYYYYMMDD(extractionData.params.endDate)}`;
            await chrome.downloads.download({ url: dataUrl, filename: `${downloadFolder}/abastecimentos_poc_${dateSuffix}.xls`, saveAs: false });
        }
...
async function handleExtractionError(system, error, config) {
    ...
    if (isAuthError) {
        chrome.notifications.create({ type: 'basic', iconUrl: 'icons/icon128.png', title: `Falha de Login no ${system}`, message: 'As credenciais parecem estar incorretas. A automação foi pausada.' });
        await chrome.alarms.clear(ALARM_SCHEDULER_CHECK);
    }
}
...
async function scheduleNextExtraction(config) {
    ...
    if (frequency === 'none') {
        await chrome.storage.local.remove(STORAGE_SCHEDULE_KEY);
    ...
    await chrome.storage.local.set({ [STORAGE_SCHEDULE_KEY]: schedule });
    
    chrome.alarms.create(ALARM_SCHEDULER_CHECK, { periodInMinutes: 60 });
...
async function runScheduledExtractionIfNeeded() {
    ...
    const { [STORAGE_CONFIG_KEY]: config, [STORAGE_SCHEDULE_KEY]: schedule } = await chrome.storage.local.get([STORAGE_CONFIG_KEY, STORAGE_SCHEDULE_KEY]);
...
export function initializeAbastecimentosBackground() {
    chrome.runtime.onInstalled.addListener(async (details) => {
        if (details.reason === 'install') {
            ...
            await chrome.storage.local.set({ [STORAGE_CONFIG_KEY]: defaultConfig });
            await addLog('Configurações padrão definidas na instalação.');
        } else if (details.reason === 'update') {
            const { [STORAGE_CONFIG_KEY]: config } = await chrome.storage.local.get(STORAGE_CONFIG_KEY);
            if (config) {
...
export function handleAbastecimentosMessages(request, sender, sendResponse) {
    const { action, settings } = request;
    
    if (action === 'saveAndValidateCredentials') {
        (async () => {
            try {
                const results = {};
                ...
                if (allSuccess) {
                    await chrome.storage.local.set({ [STORAGE_CONFIG_KEY]: settings });
                    await addLog('Configurações salvas com sucesso.', 'SISTEMA', 'success');
                    await scheduleNextExtraction(settings);
                }
                chrome.runtime.sendMessage({ action: 'validationComplete', results }).catch(()=>{});
                sendResponse({success: true, validationResults: results});
            } catch (e) {
    ...
    if (action === 'startManualExtraction') {
        (async () => {
             if (isExtractionRunning) {
                sendResponse({success: false, message: "Uma extração já está em andamento."});
                return;
            }

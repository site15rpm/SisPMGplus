    ...
    switch (action) {
        ...
        case 'sicor-fetch-units': {
            if (isExtractionRunning) {
                 ...
                 return { success: false, error: 'Uma extração de dados já está em andamento. Tente novamente em alguns instantes.' };
             }
             ...
             try {
                await addSicorLog('Buscando lista de unidades do SICOR...', 'SISTEMA');
                 ...
                 return { success: true, selectOptionsHTML: cleanedOptionsHTML };
             } catch (error) {
                 ...
                 return { success: false, error: `Falha ao buscar unidades: ${error.message}` };
             }
        }

        case 'sicor-log-feedback':
            if (payload?.message) {
        ...
        case 'sicor-extract-now':
            if (isExtractionRunning) {
                ...
                return { success: false, error: 'Uma extração já está em andamento.' };
            }
            ...
            if (typeof unidadeId === 'undefined') {
                ...
                isExtractionRunning = false;
                return { success: false, error: 'ID da unidade ausente.' };
            }
            ...
            try {
                ...
                return { success: allDownloadsSuccessful, message: finalMessage };
                ...
                isExtractionRunning = false;
                browser.runtime.sendMessage({ action: 'sicor-extraction-finished' }).catch(()=>{});
            }
    }
}
...
async function runScheduledSicorExtractionIfNeeded() {
    ...
    try {
        ...
        browser.runtime.sendMessage({ action: 'sicor-extraction-finished' }).catch(()=>{}); // Sinaliza fim para UI
    }
}
...
async function executeSicorDownload(payload) {
    ...
    try {
        ...
        if (payload.manterCopiaXls) {
            ...
            try {
                await browser.downloads.download({ url: dataUrl, filename: filename, saveAs: false });
                 await addSicorLog(`[${logPrefix}${yearSuffix}] Cópia local XLS iniciada via API.`, 'DOWNLOAD', 'success');
            } catch (downloadError) {
                 await addSicorLog(`[${logPrefix}${yearSuffix}] Falha browser.downloads (${downloadError.message}). Fallback desativado.`, 'DOWNLOAD', 'error');
